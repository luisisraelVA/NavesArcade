...
ANaveAcechadora::ANaveAcechadora()
{
    ...
    static ConstructorHelpers::FObjectFinder<UStaticMesh> ConoMesh(
    ...
    else
    {
        ...
        {
            MallaEnemigo->SetStaticMesh(CylinderMesh.Object);
        }
    }
...
void ANaveAcechadora::Tick(float DeltaTime)
{
    ...
    if (TargetJugador)
    {
        float Distancia =
            FVector::Dist(
                GetActorLocation(),
                TargetJugador->GetActorLocation()
            );

        if (Distancia > 800.0f)
        {
            FVector Direccion =
                (TargetJugador->GetActorLocation() - GetActorLocation()).GetSafeNormal();
        ...
        AddActorWorldOffset(
            FVector(
                0.0f,
                0.0f,
                FMath::Sin(GetWorld()->GetTimeSeconds() * 8.0f) *
                60.0f *
                DeltaTime
            )
        );
        ...
        if (Distancia < 600.0f)
        {
            ANaveJugador* Jugador =
                Cast<ANaveJugador>(TargetJugador);
...
{
    // Eliminar enemigo del HUD optimizado
    ANavesArcadeGameMode* GM =
        Cast<ANavesArcadeGameMode>(GetWorld()->GetAuthGameMode());
