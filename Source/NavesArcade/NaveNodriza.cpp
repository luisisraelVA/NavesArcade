...
ANaveNodriza::ANaveNodriza()
{
    ...
    {
        EfectoMuerteMasiva = ParticulaAsset.Object;
    }
}
...
void ANaveNodriza::BeginPlay()
{
    ...
    if (GameMode)
    {
        ...
        // --- DETECTAR NIVEL 9 ---
        if (GameMode->GetNivelActual() == 9)
        {
            bEsNivelNueve = true;
        }
    }

    // --- CONFIGURAR ATAQUE SEGÚN NIVEL ---
    if (bEsNivelNueve)
    {
        GetWorldTimerManager().SetTimer(
            TimerDisparoAbanico,
            this,
            &ANaveNodriza::DispararEspiralHelicoidal,
            0.08f,
            true,
            1.0f
        );
...
void ANaveNodriza::Tick(float DeltaTime)
{
    ...
    AddActorWorldOffset(
        FVector(
            0.0f,
            0.0f,
            FMath::Sin(GetWorld()->GetTimeSeconds() * 2.0f) * 80.0f * DeltaTime
        )
    );
...
void ANaveNodriza::DispararEspiralHelicoidal()
{
    ...
    if (AnguloEspiralAcumulado >= 360.0f)
    {
        AnguloEspiralAcumulado -= 360.0f;
    }
    ...
    FVector Origen =
        GetActorLocation() +
        RotacionProyectil.Vector() * 400.0f;
...
void ANaveNodriza::InvocarKamikaze()
{
    ...
    ANaveAcechadora* Kamikaze =
        GetWorld()->SpawnActor<ANaveAcechadora>(
            ANaveAcechadora::StaticClass(),
            SpawnLoc,
    ...
    if (Kamikaze)
    {
        Kamikaze->SetActorScale3D(FVector(1.2f, 1.2f, 1.2f));
    }
}
...
void ANaveNodriza::RecibirDano(float Dano)
{
    ...
    GetWorldTimerManager().SetTimer(
        TimerHitFlash,
        this,
        &ANaveNodriza::RestaurarMaterial,
        0.1f,
        false
    );
