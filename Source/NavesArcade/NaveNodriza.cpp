...
ANaveNodriza::ANaveNodriza()
{
    ...
    // Valores por defecto (se ajustarán en BeginPlay)
    VidaMaxima = 800.0f;
...
void ANaveNodriza::BeginPlay()
{
    ...
    // --- Ajustar vida según dificultad (ahora el mundo existe) ---
...
void ANaveNodriza::RecibirDano(float Dano)
{
    ...
    if (VidaActual <= 0.0f)
    {
        ...
        if (EfectoMuerteMasiva) UGameplayStatics::SpawnEmitterAtLocation(GetWorld(), EfectoMuerteMasiva, GetActorLocation(), FRotator::ZeroRotator, FVector(12.0f));
